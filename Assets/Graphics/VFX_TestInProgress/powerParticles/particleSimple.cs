...
[System.Serializable]
public class IntTfEvent : UnityEvent<int, Transform> {}

public class particleSimple : MonoBehaviour {

	public Transform target; // JUS FOR DEMO
	public int ParticlesCount; // JUST FOR DEMO
	...
	void Update () {
		...
		if(Input.GetKeyDown(KeyCode.Space)) initEvent.Invoke(ParticlesCount, target); // JUST FOR DEMO
		...
		for(int i=0; i<particleCount; i++)
		{
			...
			Vector3 newVel = ProcessParticleVelocity(pVel, pPos, target.position, i); // Boid based position
	...
	public void InitializePS (int count, Transform targetTf) {
		...
		// Creates particles
		if(count >= 50) Debug.LogError(count + " verts for new " + this.name + ". CPU said  :'(");
		else print(count + " verts for new " + this.name);
		PS.Emit(count);

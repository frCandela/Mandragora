Shader "Mandragora/volumetricShader"
{
	Properties
	{
	...
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 frag (v2f i, fixed facing : VFACE) : SV_Target
			{
				...
				intersectionValue = clamp(intersectionValue / _MaxInterDist, 0, 1); // min 0 / max 1
				float inValue = saturate((intersectionValue - _IntersectionIn) / _FadeIn); // process fade in
				float outValue = saturate(1 - ((intersectionValue - (1 - _IntersectionOut) + _FadeOut) / _FadeOut)); // process fade out
				float highLight = inValue * outValue; // combine fadeIn and fadeOut
